...
public class BookFlightWorkAreaJPanel extends javax.swing.JPanel {
    ...
    DefaultTableModel dtm = (DefaultTableModel)tbltravelagency.getModel();
        ...
        for(Aircraft aircraft:travelAgency.getAirlinerDirectory().getAirlinerList()) {
            ...
            row[0]=aircraft.getAirlinerName();
            row[1]=flight;
            row[2]=aircraft.getFlightList().get(i).getSource();
            row[3]=aircraft.getFlightList().get(i).getDestination();
            row[4]=aircraft.getFlightList().get(i).getDepartureTime();
            row[5]=aircraft.getFlightList().get(i).getArrivalTime();
            row[6]=aircraft.getFlightList().get(i).getFlightPrice();
            row[7]=aircraft.getFlightList().get(i).getAvailableSeats();
            dtm.addRow(row);
            count++;
            }
            i++;
     ...
     private void populateLocation(){
         ...
         for(Aircraft aircraft:travelAgency.getAirlinerDirectory().getAirlinerList()){
            int i=0;
            for(Flight flight:aircraft.getFlightList()) { 
                source.add(aircraft.getFlightList().get(i).getSource());
                destination.add(aircraft.getFlightList().get(i).getDestination());
                i++;
                }
            }
